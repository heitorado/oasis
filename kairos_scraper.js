const scrape = (params) => {
    ...
    const scrapedData = Array.from(columnDays).map((dayColumn) => {
        const inOutTimes = Array.from(dayColumn.children)
            .filter((hourFilled) => {
                const arrayList = Array.from(hourFilled.classList);
                return arrayList.includes(HOUR_CELL_CLASSNAME);
            })
            .map((hourText) => {
                if (hourText.children[0]) return hourText?.children[0]?.innerHTML;
            })
            .filter((el) => el != undefined);

        const inTimeList = inOutTimes.filter((_, index) => isEven(index));
        const outTimeList = inOutTimes.filter((_, index) => !isEven(index));
...
const getDate = (dayColumn) => {
    const columnDate = Array.from(dayColumn.children).filter((el) => {
        const arrayList = Array.from(el.classList);
...
const formatHour = (time) => {
    ...
    const minutes = time % ONE_HOUR_IN_MINUTE;
    return `${hour < 1 ? "00" : hour}:${
        minutes < 0 ? minutes * -1 : minutes < 10 ? "0" + minutes : minutes
    }`;
};

const formatRemainingHour = (time, remainingTime) => {
    const calculatedTime = remainingTime - time;
    if (calculatedTime < 0) return `-${formatHour(calculatedTime * -1)}`;
    return formatHour(calculatedTime);
};

const getDuration = (time) => moment.duration(time);
const isEven = (number) => number % 2 === 0;

const translateWeekDaysMap = {
    Sunday: "Domingo",
...
const LAB_BASE_WORK_HOUR_IN_MINUTE = 420;
